...
# get peak locations
df_peak_location = fread(synGet('syn25912056')$path)
...
plot_triple = function(SNP, ensGene, peakID,window, fileOut){
	...
	fig_eQTL = ggplot(df, aes(Position, log10.p.value, color=!is.na(PIP), size=isCandSet+1.0)) + geom_point() + scale_x_continuous(limits=edges, expand=c(0,0)) + scale_y_continuous(expand=c(0, 0), limits=c(0,ymax*1.03)) + scale_color_manual(values=c("black", "red")) + ylab(bquote(-log[10]~P)) + scale_size(range=c(.8, 2), breaks=c(1.1))
	...
	ymax = max(df$log10.p.value)
	fig_caQTL = ggplot(df, aes(Position, log10.p.value, color=!is.na(PIP), size=isCandSet+1)) + geom_point() + scale_x_continuous(limits=edges, expand=c(0,0)) + scale_y_continuous(expand=c(0, 0), limits=c(0,ymax*1.03)) + scale_color_manual(values=c("black", "red")) + ylab(bquote(-log[10]~P)) + scale_size(range=c(.8, 2), breaks=c(1.1))
...
plot_triple(SNP, ensGene, peakID, window = 200000, "test.pdf")

gr_peak_location[peakID]

df_caQTL[Gene==peakID,]

df_caQTL[(Variant==SNP)&(Gene==peakID)]
...
plot_triple(SNP, ensGene, peakID, "test.pdf")
...
df_peak_location[name==peakID,]

df_caQTL[(Variant==SNP)&(Peak==peakID)]
...
seqlevelsStyle(gr_peak_location) = "UCSC"  # necessary
gr_peak_location = liftOver(gr_peak_location, ch)
gr_peak_location = unlist(gr_peak_location)
